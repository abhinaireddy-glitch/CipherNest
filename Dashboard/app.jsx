...
// ║  React JSX · Claude-powered agents · CICIDS-ready           ║
// ╚══════════════════════════════════════════════════════════════╝
...
.grid6{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px;}
...
${alertRows?`<div class="sec">
  <h2 style="color:#00e5ff;font-size:13px;letter-spacing:3px;margin-bottom:14px">▸ TOP ALERT EVENTS (${Math.min(20,(report.topAlerts||[]).length)} shown)</h2>
...
${blockedRows?`<div class="sec">
  <h2 style="color:#ff2d55;font-size:13px;letter-spacing:3px;margin-bottom:14px">▸ AUTO-BLOCKED IPs (${(report.blockedIPs||[]).length} total)</h2>
...
${report.aiAnalysis?`<div class="sec">
  <h2 style="color:#bf5fff;font-size:13px;letter-spacing:3px;margin-bottom:14px">▸ AI AGENT ANALYSIS (Claude ORCHESTRATOR)</h2>
...
<div style="margin-top:32px;padding-top:16px;border-top:1px solid #142030;
  color:#2a6a7a;font-size:10px;display:flex;justify-content:space-between;letter-spacing:1px">
...
export default function CipherNest(){
  ...
  // ── AGENT LOG HELPERS
  function appendLog(agent,text,color=C.textDim,streaming=false){
    const id=Date.now()+Math.random();
    setAgentLog(p=>[{id,ts:nowUTC(),agent,text,color,streaming},...p].slice(0,80));
    return id;
  }
  ...
  async function runAnalyzer(label,score,bps,pps,syn,rst,port){
    ...
    const id=appendLog("ANALYZER","...",C.purple,true);let txt="";
    await streamClaude(
  ...
  async function runLogAnalyzer(label,score,port,srcIp){
    ...
    const id=appendLog("LOG_ANALYZER","...",C.teal,true);let txt="";
    await streamClaude(
  ...
  async function runThreatDetect(label,score,bps,pps,port){
    ...
    const id=appendLog("THREAT_DETECT","...",C.red,true);let txt="";
    await streamClaude(
  ...
  async function runOrchestrator(label,score){
    ...
    const id=appendLog("ORCHESTRATOR","...",C.gold,true);let txt="";
    await streamClaude(
  ...
  // ══════════════════════════════════════════════════════════
  // TAB RENDERERS (defined as inner functions to access state)
  // ══════════════════════════════════════════════════════════
